...
class PqrsClosedExport implements FromCollection, WithHeadings, WithTitle, WithEvents
{
    /**
    ...
    */
    public function collection()
    {
     $sql = DB::table('pqrs')
       ...
       /* $sql = DB::table('respuesta')
            ->join('pqrs', 'pqrs_id', '=', 'pqrs.id')
            ->join('contacto', 'pqrs.contacto_id', '=', 'contacto.id')
            ->join('pais', 'contacto.pais_id', '=', 'pais.id')
            ->join('provincia', 'contacto.provincia_id', '=', 'provincia.id')
            ->join('grupo', 'pqrs.grupo_id', '=', 'grupo.id')
            ->select('pqrs.id','aSuNombre','formatoRecepcion','tipoSolicitud','tipoSolicitante_id',
                'medioRespuesta','breveDescripcion','descripcion','adjuntarSoporte','contacto.contacto','pais.nombre as pais',
                'provincia.nombre as provincia','contacto.ciudad','contacto.cp','contacto.direccionFisica','contacto.email',
                'contacto.telFijo1','contacto.telFijo2','contacto.telCelular','contacto.fax','grupo.nombre','nuevaGestion','pqrs.created_at')
            ->where('pqrs.estado','=', 2)

            ->latest('respuesta.created_at')
            ->get();*/
       ...
       /* $sql = DB::table('respuesta')
            ...
            ->select('pqrs.id','aSuNombre','formatoRecepcion','tipoSolicitud','tipoSolicitante_id',
                ...
                'contacto.telFijo1','contacto.telFijo2','contacto.telCelular','contacto.fax','grupo.nombre','nuevaGestion','pqrs.created_at')
            ->where('pqrs.estado','=', 2)
            ->orderby('respuesta.created_at','DESC')->take(1)
            ->toSql();
    ...
    {
        return 'Reporte de Pqrs Cerradas';
    }
